...
solvers::base_implicit_solver<dim, spacedim>::base_implicit_solver()
{
}
...
template <int dim, int spacedim>
solvers::simple_implicit_solver<dim, spacedim>::simple_implicit_solver(
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::init_components(
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::reinit_components(
  const int update_opts)
{
  my_state = solver_state::solver_not_ready;
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::free_components(
  const int update_opts)
{
  if (update_opts & solver_update_opts::update_mat)
  {
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::push_to_global_mat(
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::push_to_rhs_vec(
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::push_to_exact_sol(
  const int *rows, const Eigen::VectorXd &vec, const InsertMode ins_mode)
{
  assert(my_state == solver_state::operators_created);
  unsigned n_rows = vec.rows();
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::finish_assemble()
{
  A.makeCompressed();
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::form_factors()
{
...
template <int dim, int spacedim>
void solvers::simple_implicit_solver<dim, spacedim>::solve_system(
  Eigen::VectorXd &sol)
{
...
template <int dim, int spacedim>
solvers::petsc_direct_solver<dim, spacedim>::petsc_direct_solver(
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::init_components(
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::reinit_components(
  const int in_solver_props, const int update_opts)
{
  free_components(update_opts);
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::free_components(
  const int update_opts)
{
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::push_to_global_mat(
  const int *rows, const int *cols, const Eigen::MatrixXd &mat,
  const InsertMode ins_mode)
{
  assert(my_state == solver_state::operators_created);
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::push_to_rhs_vec(
  const int *rows, const Eigen::VectorXd &vec, const InsertMode ins_mode)
{
  assert(my_state == solver_state::operators_created);
  unsigned n_rows = vec.rows();
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::push_to_exact_sol(
  const int *rows, const Eigen::VectorXd &vec, const InsertMode ins_mode)
{
  assert(my_state == solver_state::operators_created);
  unsigned n_rows = vec.rows();
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::finish_assemble(
  const int keys_)
{
  my_state = solver_state::assemble_is_finished;
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::form_factors()
{
  assert(my_state == solver_state::assemble_is_finished);
...
template <int dim, int spacedim>
void solvers::petsc_direct_solver<dim, spacedim>::solve_system(Vec *sol_vec)
{
  int my_rank;
...
template <int dim, int spacedim>
std::vector<double>
solvers::petsc_direct_solver<dim, spacedim>::get_local_part_of_global_vec(
...
template <int dim, int spacedim>
solvers::petsc_implicit_cg_solver<dim, spacedim>::petsc_implicit_cg_solver(
  const int in_solver_props,
  const dof_counter<dim, spacedim> &in_dof_counter,
  const MPI_Comm &in_comm)
  : my_comm(&in_comm), my_dof_counter(&in_dof_counter)
{
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::init_components(
  const int in_solver_props, const int update_opts)
{
  assert(my_state == solver_state::solver_not_ready);
  if (update_opts & solver_update_opts::update_mat)
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::reinit_components(
  const int in_solver_props, const int update_opts)
{
  free_components(update_opts);
  init_components(in_solver_props, update_opts);
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::free_components(
  const int update_opts)
{
  if ((my_state & solver_state::operators_created) ||
      (my_state & solver_state::assemble_is_finished) ||
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::push_to_global_mat(
  const int *rows, const int *cols, const Eigen::MatrixXd &mat,
  const InsertMode ins_mode)
{
  assert(my_state == solver_state::operators_created);
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::push_to_rhs_vec(
  const int *rows, const Eigen::VectorXd &vec, const InsertMode ins_mode)
{
  assert(my_state == solver_state::operators_created);
  unsigned n_rows = vec.rows();
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::push_to_exact_sol(
  const int *rows, const Eigen::VectorXd &vec, const InsertMode ins_mode)
{
  assert(my_state == solver_state::operators_created);
  unsigned n_rows = vec.rows();
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::finish_assemble(
  const int keys_)
{
  my_state = solver_state::assemble_is_finished;
  if (keys_ & solver_update_opts::update_mat)
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::form_factors()
{
  assert(my_state == solver_state::assemble_is_finished);
  //performance timing
  Timer pctimer;
  ...
  MPI_Comm_rank(*my_comm, &my_rank);
  if (my_rank == 0)
    std::cout << "Preconditioning time(ms): " << pctimer.elapsedMilliseconds() << std::endl;  
}
...
template <int dim, int spacedim>
void solvers::petsc_implicit_cg_solver<dim, spacedim>::solve_system(
  Vec *sol_vec)
{
  int my_rank;
  MPI_Comm_rank(*my_comm, &my_rank);
  ...
  if (my_rank == 0){
    std::cout << num_iter << "  " << how_ksp_stopped << std::endl;
    std::cout << "Solve time(s): " << solvertimer.elapsedSeconds() << std::endl;
  }
}
...
template <int dim, int spacedim>
std::vector<double>
solvers::petsc_implicit_cg_solver<dim, spacedim>::get_local_part_of_global_vec(
  Vec *petsc_vec, const bool destroy_petsc_vec)
{
  IS from, to;
  Vec local_petsc_vec;

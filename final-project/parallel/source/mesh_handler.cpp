...
template <int dim, int spacedim> void mesh<dim, spacedim>::write_grid()
{
...
template <int dim, int spacedim> mesh<dim, spacedim>::~mesh() {}

//
//

template <int dim, int spacedim> void mesh<dim, spacedim>::init_cell_ID_to_num()
{
  unsigned n_cell = 0;
  n_owned_cell = 0;
  n_ghost_cell = 0;
...
template <int dim, int spacedim>
int mesh<dim, spacedim>::cell_id_to_num_finder(
...
template <int dim, int spacedim>
int mesh<dim, spacedim>::cell_id_to_num_finder(const std::string &cell_str_id,
                                               const bool cell_is_owned) const
{
  if (cell_is_owned)
  {
...
template <int dim, int spacedim>
template <typename F>
void mesh<dim, spacedim>::generate_mesh(F mesh_gen_func)
{
  mesh_gen_func(tria);
  write_grid();
}
...
void mesh<dim, spacedim>::refine_mesh(
  const unsigned n,
...
template <int dim, int spacedim>
void mesh<dim, spacedim>::refine_mesh_uniformly(const unsigned n)
{
  tria.refine_global(n);
  refn_cycle += n;
}
...
template <int dim, int spacedim> void mesh<dim, spacedim>::free_container()
{

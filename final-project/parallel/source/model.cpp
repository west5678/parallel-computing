...
model<ModelEq, dim, spacedim>::model(const mesh<dim, spacedim> &in_mesh)
  : my_mesh(&in_mesh)
{
}
...
template <typename ModelEq, int dim, int spacedim>
model<ModelEq, dim, spacedim>::~model()
{
}
...
void model<ModelEq, dim, spacedim>::init_model_elements(BasisType *basis)
{
...
CellManagerType *model<ModelEq, dim, spacedim>::get_owned_cell_manager(
...
const BasisType *model<ModelEq, dim, spacedim>::get_owned_cell_basis(
  const InputType &cell_id) const
{
  int num_id = my_mesh->cell_id_to_num_finder(cell_id, true);
...
template <typename ModelEq, int dim, int spacedim>
template <typename CellManagerType, typename InputType>
CellManagerType *model<ModelEq, dim, spacedim>::get_ghost_cell_manager(
  const InputType &cell_id) const
{
...
template <typename ModelEq, int dim, int spacedim>
void model<ModelEq, dim, spacedim>::free_containers()
{

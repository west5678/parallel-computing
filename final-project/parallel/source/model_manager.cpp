...
hybridized_model_manager<dim, spacedim>::hybridized_model_manager()
{
}
...
void hybridized_model_manager<dim, spacedim>::form_dof_handlers(
  model<ModelEq, dim, spacedim> *in_model, BasisType *in_basis)
{
...
void hybridized_model_manager<dim, spacedim>::apply_on_owned_cells(
...
template <int dim, int spacedim>
template <typename ModelEq, typename Func, typename... Args>
void hybridized_model_manager<dim, spacedim>::apply_on_ghost_cells(
  model<ModelEq, dim, spacedim> *in_model, Func f, Args... args)
{
  for (std::unique_ptr<cell<dim, spacedim> > &i_cell :
...
void hybridized_model_manager<dim, spacedim>::connect_to_other_model(

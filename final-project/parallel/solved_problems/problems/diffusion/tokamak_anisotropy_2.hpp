...
struct problem_data_2 : public diffusion<dim, spacedim>::data
{
  /**
   * @brief pi
   */
   ...
   * @brief Constructor.
   */
  problem_data_2() : diffusion<dim, spacedim>::data() {}
...
{
  typedef diffusion<dim> ModelEq;
  typedef model<ModelEq, dim> ModelType;
  typedef typename ModelEq::hdg_polybasis BasisType;
  typedef typename diffusion<dim>::template hdg_manager<BasisType>
    CellManagerType;
  ...
  static void run(int argc, char **argv)
  {
...

    {
      ...
      implicit_hybridized_numbering<dim> dof_counter1;
      hybridized_model_manager<dim> model_manager1;
      ...
      for (unsigned i_cycle = 0; i_cycle < 1; ++i_cycle)
      {
        ...
        //
        int solver_keys = solvers::solver_props::spd_matrix;
        int update_keys = solvers::solver_update_opts::update_mat |
                          solvers::solver_update_opts::update_rhs;
        
	timer.start();
	//
        // PETScWrappers::SolverGMRES solver(solver_control, mpi_communicator);
        solvers::petsc_implicit_cg_solver<dim> solver1(solver_keys,
                                                       dof_counter1, comm);

        // solvers::petsc_direct_solver<dim> solver1(solver_keys,
        // 						  dof_counter1,
        //						  comm);
        //
        model_manager1.apply_on_owned_cells(
	...
	timer.stop();
        ...
        //
        distributed_vector<dim> dist_sol_vec(model_manager1.local_dof_handler,
                                             PETSC_COMM_WORLD);
        distributed_vector<dim> dist_refn_vec(model_manager1.refn_dof_handler,
                                              PETSC_COMM_WORLD);
        //
        model_manager1.apply_on_owned_cells(
        ...
        if (comm_rank == 0)
        {
          std::cout << sqrt(u_error_global) << " " << sqrt(q_error_global)
                    << std::endl;
	  std::cout << "Total Petsc solve time(s): " << timer.elapsedSeconds() << std::endl;
        }

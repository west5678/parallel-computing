...
#include "include/mesh_handler.hpp"
...
struct diffusion_data_2 : public diffusion<dim, spacedim>::data
{
  /**
   * @brief pi
   */
   ...
   * @brief Constructor.
   */
  diffusion_data_2() : diffusion<dim, spacedim>::data() {}
...
{
  typedef diffusion<dim> DiffEq;
  typedef model<DiffEq, dim> DiffModel;
  typedef typename DiffEq::hdg_polybasis DiffBasis;
  typedef typename diffusion<dim>::template hdg_manager<DiffBasis>
    DiffManagerType;
  ...
  static void assign_diff_BCs(DiffManagerType *in_manager)
  {
    ...
    for (unsigned i_face = 0; i_face < 2 * dim; ++i_face)
    {
      ...
      if (face->at_boundary())
      {
        ...
        {
          in_manager->BCs[i_face] = diffusion<dim>::boundary_condition::natural;
          in_manager->dof_status_on_faces[i_face].resize(n_dof_per_face, 1);
        }
        else
        {
          in_manager->BCs[i_face] =
            diffusion<dim>::boundary_condition::essential;
  ...
  static void run(int argc, char **argv)
  {
...

    {
      ...
      mesh<dim> mesh1(comm, 1, true);

      diffusion_data_2<dim> data1;

      mesh1.generate_mesh(rect_mesh_gen_1);
      DiffBasis basis0(3, 4);
      implicit_hybridized_numbering<dim> dof_counter0;
      hybridized_model_manager<dim> model_manager0;
      ...
      for (unsigned i_cycle = 0; i_cycle < 1; ++i_cycle)
      {
        ...
        //
        int solver_keys0 = solvers::solver_props::default_option;
        int update_keys0 = solvers::solver_update_opts::update_mat |
                           solvers::solver_update_opts::update_rhs;
        //
        //         solvers::petsc_implicit_cg_solver<dim> solver1(
        //           solver_keys, dof_counter1, comm);
        solvers::petsc_direct_solver<dim> solver0(solver_keys0, dof_counter0,
                                                  comm);
        model_manager0.apply_on_owned_cells(
        ...
        //
        distributed_vector<dim> dist_sol_vec0(model_manager0.local_dof_handler,
                                              PETSC_COMM_WORLD);
        distributed_vector<dim> dist_refn_vec(model_manager0.refn_dof_handler,
                                              PETSC_COMM_WORLD);
        //
        model_manager0.apply_on_owned_cells(
